...
def cast_spoil(ballot_id: str, do_cast: bool, ballots_encrypted: List, ballot_box: BallotBox, store: BallotStore,
               metadata: InternalElectionDescription, context: CiphertextElectionContext) -> bool:
    """
    :param ballot_id:
    ...
    if do_cast:
        accepted_ballot = accept_ballot(
            ballot, BallotBoxState.CAST, metadata, context, store
        )
        assert (store.get(accepted_ballot.object_id) == accepted_ballot)
        return True, store
    else:
        accepted_ballot = ballot_box.spoil(ballot)
        assert (store.get(accepted_ballot.object_id) == accepted_ballot)
        return True, store
